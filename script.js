...
const poseSides = ['right', 'right', 'left', 'left'];
...
function resetScores() {
  scores = Object.fromEntries(Object.keys(characters).map(key => [key, 0]));
}
...
function renderQuestion() {
  ...
  $('progress-count').textContent = `${qIndex + 1} / ${questions.length}`;
  $('progress-fill').style.width = `${((qIndex + 1) / questions.length) * 100}%`;
  ...
  answers.forEach(([answerEnglish, answerKorean, targets]) => {
    ...
    button.innerHTML = `${answerEnglish}<small>${answerKorean}</small>`;
    button.addEventListener('click', () => choose(targets));
...
function choose(targets) {
  ...
  qIndex += 1;
  qIndex < questions.length ? renderQuestion() : showAnalysis();
}

function selectResult() {
  const maximumScore = Math.max(...Object.values(scores));
  const finalists = Object.keys(scores).filter(key => scores[key] === maximumScore);
  selected = finalists[Math.floor(Math.random() * finalists.length)];
}
...
function showResult() {
  ...
  $('result-image').src = `images1/${character.folder}/book.png?v=20260731`;
...
function openBooth() {
  ...
  const character = characters[selected];
  $('photo-title').textContent = `Take Photos with ${character.name}`;
...
function updatePose() {
  ...
  overlay.src = `images1/${character.folder}/${poses[poseIndex]}?v=20260731`;
...
async function captureWithCountdown() {
  if (!stream || shot >= 4 || countingDown) return;
  ...
  if (shot < 4) $('capture-btn').disabled = false;
}
...
function getVisibleBounds(image) {
  ...
  const pixels = tempContext.getImageData(
    0,
    0,
    tempCanvas.width,
    tempCanvas.height
...
function drawCharacter(context, overlay, side, canvasWidth, canvasHeight) {
  ...
  const drawX =
    side === 'left'
      ? sideMargin
      : canvasWidth - drawWidth - sideMargin;

  const drawY = canvasHeight - drawHeight - bottomMargin;
...
function captureFrame() {
  ...
  context.scale(-1, 1);
  drawVideoCover(context, video, 0, 0, canvas.width, canvas.height);
  context.restore();

  const poseIndex = Math.min(shot, 3);
  const side = poseSides[poseIndex];

  drawCharacter(context, overlay, side, canvas.width, canvas.height);

  const photoUrl = canvas.toDataURL('image/jpeg', 0.93);
  photos.push(photoUrl);

  const preview = document.createElement('img');
  preview.src = photoUrl;
  preview.alt = `Photo ${shot + 1}`;
  $('preview-grid').appendChild(preview);
...
async function saveStrip() {
  ...
  const sideMargin = 48;
  const gap = 22;
  const photoWidth = (canvasWidth - sideMargin * 2 - gap) / 2;
  const photoHeight = 600;
  const headerHeight = 170;
  const footerHeight = 160;
  const canvasHeight =
    headerHeight + photoHeight * 2 + gap + footerHeight;

  canvas.width = canvasWidth;
  canvas.height = canvasHeight;

  context.fillStyle = '#fbf4ff';
  ...
  const positions = [
    ...
    [sideMargin + photoWidth + gap, headerHeight],
    [sideMargin, headerHeight + photoHeight + gap],
    [sideMargin + photoWidth + gap, headerHeight + photoHeight + gap]
  ];

  loadedImages.forEach((image, index) => {
    const [photoX, photoY] = positions[index];

    context.fillStyle = '#d9ccef';
    roundedRectPath(
      context,
      photoX - 4,
      photoY - 4,
...
function resetBoothForRetake() {
  ...
  if ($('preview-grid')) $('preview-grid').innerHTML = '';
  if ($('save-strip-btn')) $('save-strip-btn').classList.add('hidden');
  if ($('capture-btn')) $('capture-btn').disabled = !stream;
...
$('photo-btn').addEventListener('click', openBooth);
$('camera-btn').addEventListener('click', startCamera);
$('capture-btn').addEventListener('click', captureWithCountdown);
$('save-strip-btn').addEventListener('click', saveStrip);

$('back-result-btn').addEventListener('click', goBackToResult);

$('back-result-bottom-btn').addEventListener('click', goBackToResult);

$('retake-btn').addEventListener('click', resetBoothForRetake);
